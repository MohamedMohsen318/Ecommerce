...
@section('content')
    <h2 class="mb-2 text-xl font-semibold text-stone-900">Order #{{ $order->id }}</h2>
    <p class="text-stone-500">Customer: {{ $order->user->name }} ({{ $order->user->email }})</p>
    <p class="text-stone-500">Shipping to: {{ $order->shipping_address }}</p>
    <p class="mt-2">
    ...
    <div class="mt-8 grid gap-8 lg:grid-cols-3">
        <div class="lg:col-span-2 space-y-3">
            <h3 class="font-medium text-stone-900">Items</h3>
            ...
            <div class="space-y-1 pt-2 text-right">
                <p class="text-stone-500">Subtotal: {{ number_format($order->subtotal, 2) }}</p>
                @if ($order->discount_amount > 0)
                    <p class="text-stone-500">Discount ({{ $order->discount?->code }}): -{{ number_format($order->discount_amount, 2) }}</p>
                @endif
                <p class="text-lg font-semibold text-stone-900">Total: {{ number_format($order->total, 2) }}</p>
            </div>

            <h3 class="pt-6 font-medium text-stone-900">Status history</h3>
            <div class="space-y-2">
                @foreach ($order->statusHistory as $history)
                    <div class="rounded-lg border border-stone-200 bg-white p-3 text-sm">
                        ...
                        @if ($history->admin)
                            <span class="text-stone-500"> &middot; by {{ $history->admin->name }}</span>
                        @endif
                        @if ($history->note)
        ...
        <div>
            @if ($nextStatuses->isNotEmpty())
                <form method="POST" action="{{ route('admin.orders.update-status', $order) }}" class="space-y-3 rounded-xl border border-stone-200 bg-white p-4">
                    @csrf @method('PATCH')
                    <div>
                        <label for="status" class="block text-sm font-medium text-stone-700">Change status to</label>
                    ...
                    <div>
                        <label for="note" class="block text-sm font-medium text-stone-700">Note (optional)</label>
            ...
            @else
                <p class="text-sm text-stone-500">No further status changes possible.</p>
            @endif
        </div>
    </div>
@endsection
